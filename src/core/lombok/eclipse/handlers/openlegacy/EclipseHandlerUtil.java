...
public class EclipseHandlerUtil {
	...
	public static void generateLombokData(EclipseNode typeNode, EclipseNode annotationNode) {
		...
		new HandleConstructor().generateRequiredArgsConstructor(typeNode, AccessLevel.PUBLIC, null, HandleConstructor.SkipIfConstructorExists.YES, Collections.<Annotation>emptyList(), annotationNode);
		
	}
	
	/**
	 * The method checks if the annotation is set over class and not over interface,
	 * annotation or enum
	 */
	 ...
	 *
	 * @param fields
	 *            {@link FieldDeclaration} array
	 * @param fieldName
	 *            Sting
	 * @return boolean result
	 */
	public static boolean fieldExist(FieldDeclaration[] fields, String fieldName) {
		if (fields == null || fields.length == 0 || fieldName == null || fieldName.trim().isEmpty()) return false;
		
		for (FieldDeclaration declaration : fields)
			if (fieldName.equals(String.valueOf(declaration.name))) return true;
	...
	public static boolean supportTerminalData(Annotation[] annotations) {
		...
		for (Annotation annotation : annotations) {
			...
			String screenEntityName = annotation.type instanceof SingleTypeReference ? ScreenEntity.class.getSimpleName() : ScreenEntity.class.getName();
		...
		if (ann != null) {
			...
			for (MemberValuePair pair : pairs) {
				if ("supportTerminalData".equals(new String(pair.name))) {
					...
					if (FalseLiteral.class.isInstance(expression)) return false;
					return true;
				}
			}
		}
	 ...
	 *
	 * @param interfaces
	 *            This is array of interfaces which will be added to typeDeclaration
	 * @param typeNode
	 *            This is the node of type which interfaces list will be modified
	 */
	...
	/**
	 * The method checks if inner class with {
	 * 
	 * @param className
	 *            String} exists in the enclosing class {
	 * @param typeNode
	 *            EclipseNode}
	 *
	 * @return boolean result
	...
	public static boolean classExists(EclipseNode typeNode, String className) {
		for (EclipseNode node : typeNode.down())
			if (node.getKind() == AST.Kind.TYPE && className.equals(node.getName())) return true;
	 ...
	 * 
	 * @param newAnnotation
	 *            Annotation array} and {
	 * @param annoataion
	 *            Annotation} to the new Annotation array
	 *
	...
	/**
	 * returns true in the case passed typeReference is a representation of OL
	 * primitive types that are not inner or external class types
	 */
	public static boolean isPrimitive(TypeReference typeReference) {
		...
		if (Eclipse.nameEquals(typeReference.getTypeName(), String.class.getSimpleName()) || Eclipse.nameEquals(typeReference.getTypeName(), String.class.getName())) {
			return true;
		}
		if (Eclipse.nameEquals(typeReference.getTypeName(), BigInteger.class.getSimpleName()) || Eclipse.nameEquals(typeReference.getTypeName(), BigInteger.class.getName())) {
			return true;
		}
		if (Eclipse.nameEquals(typeReference.getTypeName(), Integer.class.getSimpleName()) || Eclipse.nameEquals(typeReference.getTypeName(), Integer.class.getName())) {
			return true;
		}
		if (Eclipse.nameEquals(typeReference.getTypeName(), Boolean.class.getSimpleName()) || Eclipse.nameEquals(typeReference.getTypeName(), Boolean.class.getName()) || Eclipse.nameEquals(typeReference.getTypeName(), "boolean")) {
			return true;
		}
		if (Eclipse.nameEquals(typeReference.getTypeName(), Date.class.getSimpleName()) || Eclipse.nameEquals(typeReference.getTypeName(), Date.class.getName())) {
			return true;
		}
		if (Eclipse.nameEquals(typeReference.getTypeName(), Double.class.getSimpleName()) || Eclipse.nameEquals(typeReference.getTypeName(), Double.class.getName())) {
			return true;
		}
		if (Eclipse.nameEquals(typeReference.getTypeName(), BigDecimal.class.getSimpleName()) || Eclipse.nameEquals(typeReference.getTypeName(), BigDecimal.class.getName())) {
			return true;
		}
	...
	/**
	 * method returns List of all fields annotated with {@link ScreenField}
	 * annotation
	 */
